const cart = require('./cart');
const cars = require('./data/cars')

describe('Cart Properties:', () => {
    test("Expect cart to be an empty array", () => {
        expect(Array.isArray(cart.cart)).toEqual(true);
        expect(cart.cart.length).toEqual(0);
    });
    test("Expect the 'total' property to be 0", () => {
        expect(cart.total).toEqual(0)
    })
})

describe('Cart Methods:', () => {
    afterEach(() => {
        cart.cart = [];
        cart.total = 0;
    })
    test("addToCart() should add a car object onto the cart array", () => {
        cart.addToCart(cars[0]);
        cart.addToCart(cars[1]);
        expect(cart.cart.length).toEqual(2);
        expect(cart.cart[0]).toEqual(cars[0]);
        expect(cart.cart[1]).toEqual(cars[1]);
        
    })
    test("addToCart() should increase the total property", () => {
        cart.addToCart(cars[0]);
        cart.addToCart(cars[7]);
        cart.addToCart(cars[4]);
        expect(cart.total).toEqual(cars[0].price + cars[7].price + cars[4].price);
    })
    test("removeFromCart() should remove a car object from the cart array", () => {
        cart.addToCart(cars[0]);
        cart.addToCart(cars[4]);
        cart.addToCart(cars[8]);
        //The first parameter in removeFromCart is the objects index, the second is the objects price property. They allow us to quickly splice the object and decrease the total by the chosen car's price. //
        cart.removeFromCart(1, cars[4].price)

        expect(cart.cart.length).toEqual(2);
        expect(cart.cart[0]).toEqual(cars[0]);
        expect(cart.cart[1]).toEqual(cars[8]);
    })
    test("removeFromCart() should decrease the total property", () => {
        cart.addToCart(cars[0]);
        cart.addToCart(cars[4]);
        cart.addToCart(cars[8]);

        cart.removeFromCart(0, cars[0].price);
        cart.removeFromCart(1, cars[4].price);

        expect(cart.total).toEqual(cars[8].price);
    })
    test("checkout() should empty the cart array and set total to 0", () => {
        cart.addToCart(cars[0]);
        cart.addToCart(cars[3]);
        cart.addToCart(cars[8]);
        cart.addToCart(cars[2]);

        cart.checkout();

        expect(cart.cart.length).toEqual(0);
        expect(cart.total).toEqual(0);
    })
})